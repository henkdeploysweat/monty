...
--
-- TIMEZONE: OCCURRED_AT is TIMESTAMP_NTZ in the ingestion source zone
-- (%(src_tz)s, default 'America/Los_Angeles'). Ordering/windowing use the raw
-- column (pruning-friendly, and order is preserved under conversion); only the
-- output latest_at is converted to UTC.
--
-- MAD is a TWO-STAGE aggregation (median of |x - median(x)|); a window
-- MEDIAN() OVER cannot nest inside an aggregate MEDIAN() in one GROUP BY, so
-- it is split into `med` then `mad`.
...
WITH series AS (
    ...
    WHERE ENVIRONMENT = %(env)s
      ...
      AND OCCURRED_AT >= CONVERT_TIMEZONE('UTC', %(src_tz)s,
                            DATEADD('day', -%(baseline_days)s, SYSDATE()))
...
SELECT
    ...
    l.PIPELINE_NAME,
    l.latest_value,
    CONVERT_TIMEZONE(%(src_tz)s, 'UTC', l.latest_at)             AS latest_at,  -- -> UTC
