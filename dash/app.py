...
@app.route("/segment")
def segment():
    """Search page over every custom event landing in the Segment warehouse."""
    q = request.args.get("q", "").strip()
    error = None
    catalog = []
    results = None
    try:
        catalog = db.fetch_segment_catalog()
        if q:
            results = db.fetch_segment_search(q)
    except Exception as exc:  # surface connection/query issues in-page, not a 500
        error = str(exc)
    total_events = len(catalog)
    total_sources = len({s for row in catalog for s in row.get("SOURCES", [])})
    state = db.segment_source_state()
    spec = db.fetch_combined_spec()          # documented tracking-plan tab
    return render_template("segment.html",
                           q=q, catalog=catalog, results=results, error=error,
                           total_events=total_events, total_sources=total_sources,
                           segment_db=db.SEGMENT_DB,
                           segment_mode=state["mode"],          # snowflake | cache | sample
                           segment_synced=state["synced_at"],
                           spec=spec)


@app.route("/api/segment/save", methods=["POST"])
def api_segment_save():
    """Snapshot the segment tracking plan from Snowflake to a local CSV; the
    page serves from it afterwards. Same handler backs Save and Resync."""
    try:
        rows = db.save_segment_snapshot()
        return jsonify({"ok": True, "rows": rows, "path": str(db.SEGMENT_CACHE)})
    except Exception as exc:
        return jsonify({"ok": False, "error": str(exc)}), 502


@app.route("/api/segment/resync", methods=["POST"])
def api_segment_resync():
    """Re-pull from Snowflake and overwrite the saved snapshot (refresh)."""
    return api_segment_save()


@app.route("/api/segment/live", methods=["POST"])
def api_segment_live():
    """Drop the snapshot and go back to querying Snowflake live."""
    return jsonify({"ok": True, "cleared": db.clear_segment_snapshot()})


# --- JSON endpoints (optional; handy for auto-refresh without full reload) ---
@app.route("/api/segment/catalog")
def api_segment_catalog():
    """Full event catalog (who-sends-what matrix) as JSON."""
    try:
        return jsonify({"catalog": db.fetch_segment_catalog()})
    except Exception as exc:
        return jsonify({"error": str(exc)}), 502


@app.route("/api/segment/search")
def api_segment_search():
    """Events matching ?q= by event name or property name."""
    q = request.args.get("q", "").strip()
    try:
        return jsonify({"q": q, "results": db.fetch_segment_search(q)})
    except Exception as exc:
        return jsonify({"error": str(exc)}), 502


@app.route("/api/segment/properties")
def api_segment_properties():
    """Full property list for one ?event= across all sources that emit it."""
    event = request.args.get("event", "").strip()
    try:
        return jsonify({"event": event,
                        "properties": db.fetch_segment_properties(event)})
    except Exception as exc:
        return jsonify({"error": str(exc)}), 502
