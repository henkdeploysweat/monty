...
- **Anomaly detection** (`/anomalies`) — a robust statistical detector that
  flags metrics whose values deviate from their own recent baseline.
...
| `OCCURRED_AT` | TIMESTAMP | **UTC.** The single source of truth for time |
...
- `OCCURRED_AT` is stored in UTC. All display-timezone conversion happens at the
  Python layer (§6.4); the database and every query stay in UTC.
...
```sql
SELECT ID, PIPELINE_NAME, METRIC_NAME, METRIC_VALUE, SEVERITY,
       RUN_ID, PAYLOAD, OCCURRED_AT, IS_ALERT, SENT_TO_SLACK, SENT_AT, ENVIRONMENT
FROM {{table}}
...
- `{{table}}` — a literal string replaced by `db.py` *before* the query is sent
  (it is your `MONTY_TABLE`, e.g. `MONITORING.PUBLIC.MONTY_EVENTS`). This is
  string interpolation, not a bind parameter, because you cannot bind an
  identifier in Snowflake. It comes from a trusted config value, never user
  input — keep it that way (see §7.9 security note).
- `%(env)s` — bind parameter, `'prod'` or `'dev'`.
- `%(lookback_days)s` — bind parameter, integer number of days of history.

**The `CURRENT_TIMESTAMP()` override.** By default the window ends "now"
(`CURRENT_TIMESTAMP()`). When a caller needs a historical or reproducible window
(archive day, custom range, tests), `db.py` rewrites `CURRENT_TIMESTAMP()` to a
bound `%(now)s` parameter and passes the anchor. This is why the same query
serves live, archive-day, and range views — only the anchor and lookback change.
